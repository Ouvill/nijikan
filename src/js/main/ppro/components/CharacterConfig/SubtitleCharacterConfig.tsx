...
export function SubtitleCharacterConfig(props: {
  ...
  const onChangeSubtitleSwitch = (e: React.ChangeEvent<HTMLInputElement>) => {
    props.setCharacter({
      ...props.character,
      enableSubtitle: e.target.checked,
    });
  };
  ...
  return (
    <div className={"flex flex-col gap-2 border p-2"}>
      <div className={"flex justify-between items-center"}>
        <h2 className={"text-xl"}>字幕設定</h2>
        <Switch
          onChange={onChangeSubtitleSwitch}
          checked={props.character.enableSubtitle}
        ></Switch>
      </div>
      {props.character.enableSubtitle && (
